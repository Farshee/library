 ...
 $result = "";
 for ($i = 1; $i <= $n; $i++) { $result .=substr($generator, (rand() % (strlen($generator))), 1); } return $result; } public function get_payment() { $amount='10' ; $methodname='IB' ; $paymentmethodid='1' ; $description='test' ; $patient_id='1' ; $paymentinfo=$this->db->table('payment_settings')->get()->getRow();
 	...
 	$marchandid = $paymentinfo->store_id;
 	$islive = $paymentinfo->ssl_live;
 	$currency = '$';
 	...
 	$post_data['success_url'] = base_url('api/successpayments/');
 	$post_data['fail_url'] = base_url('api/fails/') . $info->trsnsid;
 	$post_data['cancel_url'] = base_url('api/cancel/') . $info->trsnsid;
 	...
 	$this->session->set('tarndata', $session);
