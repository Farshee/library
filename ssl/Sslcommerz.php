...
namespace App\Libraries;
	...
	class Sslcommerz
	{
		...
		//$CI =& get_instance(); 
		//$settings = $CI->db->select("*")->from('paymentsetup')->get()->row();
			$this->session = session();
			$this->db = db_connect();
			$paymentinfo     = $this->db->table('payment_settings')->get()->getRow();
	        $this->setSSLCommerzMode(1);
	        $this->store_id = $paymentinfo->store_id; //'bdtas60d9a1a656c0a';
	        $this->store_pass = $paymentinfo->password; //'bdtas60d9a1a656c0a@ssl';
